...
impl DeviceTree<'_> {
    ...
    pub fn reserved_memory_map(&self) -> Option<Node> {
        let root = self.root();
...
pub trait FdtNode {
    ...
    #[inline]
    fn props(&self) -> FdtProps {
        FdtProps::new(self.node().tokens())
    }

    fn get_prop(&self, prop_name: PropName) -> Option<FdtProperty> {
    ...
    fn find_first_child(&self, prefix: NodeName) -> Option<Node> {
        let mut level = 0;
    ...
    fn find_child_exact(&self, node_name: NodeName) -> Option<Node> {
        let mut level = 0;
        let mut iter = self.node().tokens();
        while let Some(token) = iter.next() {
            match token {
    ...
    #[inline]
    fn children(&self) -> FdtChildNodes {
        let address_cells = self.node().address_cells().unwrap_or(0);
        let size_cells = self.node().size_cells().unwrap_or(0);
...
impl<'a> Node<'a> {
    ...
    pub fn reg(&'a self) -> Option<impl Iterator<Item = (u64, u64)> + 'a> {
        self.get_prop(PropName::REG).map(|v| {
            AddressAndSizeIter::new(
                v.words(),
...
impl RootNode<'_> {
    ...
    pub fn address_cells(&self) -> usize {
        self.node().address_cells().unwrap_or_default() as usize
    }

    #[inline]
    pub fn size_cells(&self) -> usize {
        self.node().size_cells().unwrap_or_default() as usize
    }
    ...
    /// TODO: multiple nodes
    #[inline]
    pub fn memory(&self) -> Option<Node> {
        self.node().find_first_child(NodeName::MEMORY)
    }

    #[inline]
    pub fn reserved_memory(&self) -> Option<Node> {
        self.node().find_first_child(NodeName::RESERVED_MEMORY)
    }

    #[inline]
    pub fn chosen(&self) -> Option<ChosenNode> {
        self.node()
...
impl<'a> AddressAndSizeIter<'a> {
    #[inline]
    fn new(slice: &'a [BeU32], address_cells: usize, size_cells: usize) -> Self {
